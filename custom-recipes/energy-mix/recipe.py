...
for index, x in enumerate(uniquelatlon):
 ...
 #Drop and rename df columns before joining
    ...
    output_df = pd.merge_asof(input_df.sort_values(by=[DateColName]), df_col.sort_values(by=['e-mix_date_time']), by=[lat,lon], left_on=[DateColName], right_on=['e-mix_date_time'])
